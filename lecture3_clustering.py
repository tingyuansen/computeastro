...
def show_page():
    ...
    st.markdown(r'''''')
    st.markdown(r'''We'll discuss two main clustering algorithms today. We'll start with the K-means algorithm, which is simple and scalable but lacks a strong probabilistic foundation. Then we'll move on to Gaussian Mixture Models, which are more statistically rigorous and better suited for probabilistic inference in astronomy.''')
    st.markdown(r'''''')
    st.markdown(r'''Now, I'll be honest - I'm not a big fan of K-means because it doesn't align well with the statistical probabilistic approach to inferences in astronomy that we want to promote in this course. However, understanding K-means, especially its optimization through the Expectation-Maximization algorithm, will help us grasp the more complex Gaussian mixture models. So we'll use K-means as a stepping stone to build intuition.''')
    st.markdown(r'''''')
    st.markdown(r'''Let's consider a relevant example for the Research School of Astronomy and Astrophysics: distinguishing between kangaroos and wallabies based on their length. Imagine we measure the lengths of these animals and plot the distribution. We might observe two peaks: one for shorter wallabies and another for longer kangaroos.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Unsupervised learning is crucial in astronomy because we often lack labels for our data. Many astronomical pursuits involve determining the number of types of objects we have - like quasars or stars - or discovering entirely new classes based on unexplored feature spaces. While unsupervised learning may not always provide ground truth, it's invaluable for building intuition and generating hypotheses in astronomical research.''')
    st.markdown(r'''''')
    st.markdown(r'''So, while it might not give us definitive answers, unsupervised learning is a powerful tool in astronomy. It helps us formulate hypotheses and explore our data in ways that can lead to new discoveries. That's why we're dedicating time to understand these methods.''')
    st.markdown(r'''''')
    st.markdown(r'''Now, let's delve deeper into unsupervised learning and its applications in astronomy. As we mentioned, the key difference between unsupervised learning, which we'll cover in this lecture and the next, and the supervised learning methods like linear regression and logistic regression that we discussed in the last two lectures, is the absence of target labels $Y$. In unsupervised learning, we only have the features to work with.''')
    st.markdown(r'''''')
    st.markdown(r'''These features, which we denote as $X$, can be high-dimensional. In astronomical contexts, $D$, the number of dimensions, can be very large. For instance, when dealing with spectra, each individual $X$ might represent thousands or even tens of thousands of dimensions. Our goal here is to partition all these features into meaningful clusters.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''For the purposes of building intuition, including in today's tutorial, we'll limit ourselves to 2D examples. But keep in mind that these methods can be easily generalized to very high dimensions.''')
    st.markdown(r'''''')
    st.markdown(r'''Now, let's talk about a particular use case in astronomy: looking for substructure in the kinematic space of stars. This is an area I've been involved in, and it's become quite common in galactic archaeology, which is a significant part of the research here at the Research School of Astronomy and Astrophysics (RSAA).''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Initialization: Since we don't know how to describe the clusters beforehand, we arbitrarily choose some locations in our feature space to serve as initial cluster centers.''')
    st.markdown(r'''''')
    st.markdown(r'''Assignment: Given these initial centers, we assign each data point to the nearest cluster center.''')
    st.markdown(r'''''')
    st.markdown(r'''Update: After making these assignments, we update the description of each cluster. Specifically, we recalculate the cluster centers as the mean of all data points assigned to that cluster.''')
    st.markdown(r'''''')
    st.markdown(r'''Iteration: We then iterate between the assignment and update steps. We reassign points based on the updated cluster centers, then update the centers based on the new assignments.''')
    st.markdown(r'''''')
    st.markdown(r'''Convergence: We continue this process until we reach convergence. Convergence here means that further repetitions of this process no longer change either the centers of the clusters or the assignment of data points to clusters.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''By doing this, we are really compressing the information by a lot. The only floats I need to save are the mean vectors of the $K$ clusters ($K \times 3 \times$ float64). This is a very small number. I still need to save an array of $N$ pixels, but for each pixel I only need to save the cluster assignment. With 10 clusters, I can save that as an integer (4 bits).''')
    st.markdown(r'''''')
    st.markdown(r'''For most pixels, I can save the information with a very small number of bits, yet it gives a very good representation of the image. You can argue that this is somewhat an artistic view. It's a fun application - with just a few lines of code, you can start doing this type of thing at home.''')
    st.markdown(r'''''')
    st.markdown(r'''That basically ends k-means. In the following, we will follow up with something more advanced called Gaussian mixture models. K-means is very good but it's a very simple model. For one, it gives hard assignments, meaning there's no probabilistic interpretation. A good part of what we try to do in these courses is to give the probabilistic view of statistics and how we do that in astronomy.''')
    st.markdown(r'''''')
    st.markdown(r'''The simplest probabilistic model we can do for clustering is a mixture of models. A mixture model means that instead of describing the distribution of features $x$ with a single Gaussian, which would be quite limiting, we can write it as the sum of different Gaussians:''')
    st.markdown(r'''''')
    st.markdown(r'''$p(x) = \sum_{k=1}^K \pi_k \mathcal{N}(x | \mu_k, \Sigma_k)$''')
    st.markdown(r'''''')
    st.markdown(r'''Let's say I have a distribution of features $x_1$ and $x_2$ that looks like the green shape. Fitting this with a single Gaussian would not do a very good job. But if I am allowed to describe it by the sum of three Gaussians, it will probably do a pretty good job.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Now, let's dive into the optimization of Gaussian Mixture Models (GMMs). As we've shown many times in these courses, the key idea is to optimize the likelihood. Our goal here is to find the correct models that will optimize the observation.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''This soft assignment leads to some interesting properties. When calculating the new mean for a cluster (let's say the kangaroo cluster), a data point that is half wallaby and half kangaroo only contributes half of its value, compared to data points that more strictly belong to the kangaroos. Another thing to note is that, strictly speaking, none of the $\gamma_{nk}$ values are exactly zero in GMMs. They can be extremely small, but not strictly zero. Therefore, all data points actually contribute to the update of the mean, even if only slightly.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Just like with $\mu_k$, different data points have different weightings for this new covariance. If a data point is really a wallaby, it shouldn't contribute much to the dispersion of the kangaroo group. This is why we weight all these covariances by the individual $\gamma_{nk}$ and then divide by the sum of the weights.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''This result is intuitively clear: to update the weights of individual modes, we sum up all the responsibilities. For example, if we want to know the relative importance of the kangaroo mode compared to the wallaby mode, we sum up all the "kangaroo-ness" of the data points (even if some points are only fractionally kangaroo) and divide by the total number of samples.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Now, given our current assumption about the clusters, we first calculate the likelihood of each data point belonging to each cluster. Let's say we're clustering kangaroos and wallabies - we might have a yellow cluster for kangaroos and a cyan one for wallabies. We assign fractional memberships to each point. This is what we call the expectation step, where we use the ratio of likelihoods to make these soft assignments.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''One interesting thing about GMMs is that they often find the correct solution even with initializations similar to those where K-means might get stuck. This is because the soft assignments in GMMs make the loss function smoother, making it less likely to get trapped in local minima. That said, it's still good practice to run multiple initializations and keep the one with the maximum likelihood.''')
    st.markdown(r'''''')
    st.markdown(r'''Now, let's talk about why GMMs are often preferable to K-means. First off, GMMs use what we call soft assignments. Remember $\gamma_{nk}$ from our equations? It can take any value between 0 and 1, unlike the hard assignments in K-means where a point either belongs to a cluster or it doesn't. This allows for more nuanced cluster memberships.''')
    st.markdown(r'''''')
    st.markdown(r'''But perhaps more importantly, GMMs are what we call generative models. This is crucial because it means we're not just clustering data - we're learning the underlying distribution. Once we have that, we can actually generate new data points that fit that distribution. It's like the difference between being able to tell apart German and Portuguese sentences, and actually being able to generate new, grammatically correct sentences in those languages.''')
    st.markdown(r'''''')
    st.markdown(r'''This generative aspect leads to another key advantage: outlier detection. Because GMMs give us a full probability distribution, we can identify data points that are poorly described by our model. In astronomy, this is huge. A significant part of the excitement in our field comes from finding new, unexpected objects - things that don't fit our current understanding. GMMs give us a powerful tool for this kind of discovery.''')
    st.markdown(r'''''')
    st.markdown(r'''GMMs are also more flexible than K-means when it comes to cluster shapes. K-means assumes spherical clusters, but GMMs can capture elliptical and more complex shapes through their covariance matrices. This often allows them to better represent real-world data distributions.''')
    st.markdown(r'''''')
    st.markdown(r'''It's worth noting that while tools like K-means and GMMs are powerful, they do have limitations, especially when we're dealing with high-dimensional data like spectra. This is where deep learning comes into play. Deep learning offers even more powerful generative models that can capture highly non-trivial distributions. Think about models like ChatGPT for text or DALL-E for images - they're dealing with incredibly complex, high-dimensional distributions.''')
    st.markdown(r'''''')
    st.markdown(r'''In astronomy, the real excitement of machine learning, particularly deep learning, lies in its potential to find outliers and capture complex, high-dimensional distributions that classical methods like GMMs struggle with. It's important to distinguish between simply applying machine learning tools to astronomical data and doing research to merge deep learning ideas with astronomical problems. The latter is where we're seeing some of the most exciting developments in our field.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''A question that came up in the original lecture that I think is worth revisiting: How do we determine the best value for $K$ in our Gaussian Mixture Models? $K$, of course, is a hyperparameter that we often need to define beforehand, but choosing the right $K$ isn't always straightforward.''')
    st.markdown(r'''''')
    st.markdown(r'''Let's start by saying there's no universally correct answer, but there are some approaches we can consider. First, it's important to understand what happens as we increase $K$. For any given $K$, we can calculate the final best likelihood of the model. As $K$ increases, the likelihood will generally improve because our model becomes more expressive. If we were to simply choose the $K$ that gives us the best likelihood, we might keep increasing $K$ indefinitely. But of course, this approach runs the risk of overfitting our data.''')
    st.markdown(r'''''')
    st.markdown(r'''So how do we strike a balance? Well, there are some established methods in statistics for this, namely the Bayesian Information Criterion (BIC) and the Akaike Information Criterion (AIC). These methods are conceptually similar to the regularization term we discussed in linear regression - remember that $\dfrac{\lambda}{2}W^2$ term? BIC and AIC work on a similar principle. They calculate the log likelihood, but as you increase $K$, they also introduce a penalty term. This penalty discourages unnecessarily complex models unless the increased complexity is really needed to explain the data.''')
    st.markdown(r'''''')
    st.markdown(r'''Now, BIC and AIC are statistically quite elegant, and they're even rigorous under certain conditions, like when you have infinite dimensions or infinite data points. However - and this is important for us astronomers - those assumptions are often too strong for the kind of data we work with in astronomy. ''')
    st.markdown(r'''''')
    st.markdown(r'''So, what do I recommend for practical astronomical work? It's actually quite straightforward: plot the log likelihood of the best model as a function of $K$. As you do this, you'll typically see that even though the log likelihood keeps increasing with $K$, it starts to plateau at some point. Look for what we call the "elbow" or "knee" of this curve - it depends on whether you're plotting minus log likelihood or log likelihood. This turning point often indicates a good choice for $K$.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Remember, the goal isn't necessarily to find the "true" number of clusters - in many astronomical contexts, the concept of a true $K$ might not even make sense. Instead, we're looking for a model complexity that balances goodness of fit with simplicity and interpretability. This visual method helps you make that judgment in a way that's often more intuitive and practical for real-world astronomical data analysis.''')
    st.markdown(r'''''')
    st.markdown(r'''''')
