...
def show_page():
    ...
    st.markdown(r'''''')
    st.markdown(r'''One of the key findings in our paper was evidence supporting the existence of what we term "hypernovae" - highly energetic supernovae that contribute a distinct direction in this chemical space. This discovery added a new dimension, quite literally, to our understanding of stellar nucleosynthesis and galactic chemical enrichment.''')
    st.markdown(r'''''')
    st.markdown(r'''While this work is now a decade old, the application of PCA in astronomy continues to evolve and yield important results. A more recent example, which I mentioned at the beginning of this course, involves a slightly more advanced technique called latent factor analysis. This method was employed in a study published just this year, focusing on quasar continua extraction - a topic some of you are exploring in your third assignment.''')
    st.markdown(r'''''')
    st.markdown(r'''In practice, analyzing quasar spectra is more complex than our earlier stellar abundance example. We're not only dealing with the observations and the intrinsic quasar continuum but also absorption from the intergalactic medium. The paper I'm referring to combined three techniques that we'll cover in upcoming lectures: PCA, Gaussian processes, and neural networks. By integrating these methods, we achieved impressive results in reconstructing quasar continua.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$S = (1/N) \sum_{n=1}^N x_nx_n^T = Cov(X, X) \in \mathbb{R}^{D\times D}$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$V_1 \equiv \mathbb{V}[z_1] = (1/N) \sum_{n=1}^N z_{1n}^2$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$V_1 = (1/N) \sum_{n=1}^N (b_1^Tx_n)^2$''')
    st.markdown(r'''$   = (1/N) \sum_{n=1}^N b_1^Tx_nx_n^Tb_1$''')
    st.markdown(r'''$   = b_1^T((1/N) \sum_{n=1}^N x_nx_n^T)b_1$''')
    st.markdown(r'''$   = b_1^TSb_1$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\partial\mathcal{L}/\partial b_1 = 2b_1^TS - 2\lambda_1b_1^T$''')
    st.markdown(r'''''')
    st.markdown(r'''And if we differentiate with respect to $\lambda_1$, we get:''')
    st.markdown(r'''''')
    st.markdown(r'''$\partial\mathcal{L}/\partial\lambda_1 = 1 - b_1^Tb_1$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Now, if all that math seemed a bit heavy, let's look at a concrete example that you're probably familiar with: the MNIST dataset. MNIST is a small sample set with 60,000 examples of handwritten digits. It's often used for digit classification tasks. Each image is 28 by 28 pixels, giving us a total of 784 pixels. This means we can treat each observation as a 784-dimensional vector.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Contrast this with your quasar spectra in assignment three. There, you start with 7,000 dimensions, but you can compress it down to just five dimensions and still capture almost all the variance. This really drives home how simple astronomy can be in terms of intrinsic dimensionality.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Here's the trick: Instead of $S = (1/N)XX^T$, we can look at $(1/N)X^TX$. This is an $N$ by $N$ matrix, but it shares the same non-zero eigenvalues as $S$. This might seem like magic, but it's actually a common trick you'll see again when we talk about Gaussian processes. It's called the kernel trick.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''2. Substituting $S = (1/N)XX^T$, we get:''')
    st.markdown(r'''''')
    st.markdown(r'''   $(1/N)XX^Tb_m = \lambda_mb_m$''')
    st.markdown(r'''''')
    st.markdown(r'''3. Now, let's multiply both sides by $X^T$ from the left:''')
    st.markdown(r'''''')
    st.markdown(r'''   $(1/N)X^TXX^Tb_m = \lambda_mX^Tb_m$''')
    st.markdown(r'''''')
    st.markdown(r'''4. Let's define $c_m \equiv X^Tb_m$. This gives us:''')
    st.markdown(r'''''')
    st.markdown(r'''   $(1/N)X^TXc_m = \lambda_mc_m$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''   $(1/N)XX^TXc_m = \lambda_mXc_m$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''   - If $N > D$: $S = (1/N)XX^T$''')
    st.markdown(r'''   - If $N < D$: Use $(1/N)X^TX$ instead''')
    st.markdown(r'''''')
    st.markdown(r'''3. Find the eigenvalues and eigenvectors:''')
    st.markdown(r'''   - If $N > D$: Calculate for $S$ directly''')
    st.markdown(r'''   - If $N < D$: Calculate for $(1/N)X^TX$, then multiply the eigenvectors by $X$ to get the eigenvectors of $S$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''In your assignments and future research, you'll find that PCA is an invaluable tool in your data analysis toolkit. It's a perfect example of how a relatively simple mathematical concept can lead to profound insights in astronomy.''')
    st.markdown(r'''''')
    st.markdown(r'''''')
