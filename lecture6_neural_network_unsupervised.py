...
def show_page():
    ...
    st.markdown(r'''Alright, let's get started. To recap what we learned last time, we delved into neural networks. We covered the basics of neural networks, particularly focusing on how we can use auto-differentiation to compute gradients and update model parameters. We primarily discussed supervised tasks, where we have an input $X$ and output $Y$, and use the non-linearity of neural networks to find flexible relationships between them.''')
    st.markdown(r'''''')
    st.markdown(r'''However, you might recall that in the first four lectures, we explored both supervised and unsupervised tasks. Today, we'll focus on how neural networks can be applied to unsupervised tasks. There are two main areas we'll concentrate on:''')
    st.markdown(r'''''')
    st.markdown(r'''1. Dimension reduction: In classical machine learning, we discussed Principal Component Analysis (PCA) as a method for reducing dimensions. We'll revisit this concept and examine the connection between autoencoders and PCA.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''In astronomy, this concept of domain transfer has found significant applications. One example from recent work involves morphing between two closely related but distinct domains: simulated spectra and observed spectra. While these domains are similar, they're not identical due to the inherent limitations of our simulation models. By using an autoencoder approach to encode high-level abstractions and then decode them back, we can autocorrect for some of these model limitations, allowing us to bridge the gap between simulation and observation more effectively.''')
    st.markdown(r'''''')
    st.markdown(r'''This technique isn't limited to spectral analysis. Another intriguing application in astronomy involves morphing between galaxy images and spectra. While it might seem counterintuitive to predict spectra from images, there are practical reasons for this approach. Images are often cheaper and easier to obtain than spectra. By forecasting potential spectral characteristics from images, astronomers can prioritize their observational strategies more efficiently. This application typically uses a Variational Autoencoder (VAE) to create the abstraction and then decode it back into the desired domain.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''The tutorial and subsequent discussion provide valuable insights into the practical implementation and applications of Mixed Density Networks (MDNs), particularly in astronomical contexts. Despite the complex theory underlying MDNs, their implementation can be surprisingly straightforward. The core of the model can be written in just a few lines of code, leveraging the power of modern deep learning frameworks. This simplicity belies the model's flexibility in capturing complex, multi-modal distributions, as demonstrated in the "moon shape" example where the model learns to represent a bimodal distribution that varies with the input.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''As noted in the lecture, this lithium abundance case is essentially a one-dimensional version of many statistical problems in astronomy. The principles demonstrated here can be scaled up to higher-dimensional problems in cosmology and other areas of astrophysics. For instance, in cosmology, we might use similar techniques to model the relationship between cosmological parameters and observable features in the cosmic microwave background or large-scale structure of the universe.''')
    st.markdown(r'''''')
    st.markdown(r'''''')
