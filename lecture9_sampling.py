...
def show_page():
    ...
    st.markdown(r'''Welcome back. Today, we will explore sampling in machine learning, building upon the concepts we've covered thus far in our exploration of the machine learning landscape.''')
    st.markdown(r'''''')
    st.markdown(r'''Let's begin with a brief recap. Up to last Tuesday, we completed our overview of the machine learning landscape, from linear regression to logistic regression, and from Gaussian processes to neural networks. This overview represents the core of what we typically consider machine learning. While neural networks dominate much of today's machine learning discourse, our broad understanding of the classical landscape remains crucial for appreciating the need for these more modern tools.''')
    st.markdown(r'''''')
    st.markdown(r'''With our previous lecture, we concluded this overview. However, we have three more lectures ahead of us. Today and in our next lecture, we'll focus on a critical topic in machine learning: sampling. Rather than delving into the specifics of sampling techniques, we'll concentrate on understanding how sampling fits into the broader machine learning framework.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''In modern computations, we often deal with incredibly complex distributions such as posterior distributions, mixture distributions, and graphical models. Many machine learning tasks boil down to estimating the expectation of a function under a distribution: $\mathbb{E}(f) = \int f(z) p(z) dz$. This is where sampling techniques become invaluable, allowing us to approximate these complex integrals efficiently.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Remember, the core idea is this: $\mathbb{E}(f) = \int f(z) p(z) dz$. We're approximating this expectation by the sample mean of a function of simulated random variables. In mathematical terms:''')
    st.markdown(r'''''')
    st.markdown(r'''$\mathbb{E}(f) \approx (1/L) * \sum_{l=1}^L f(z^{(l)})$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Let's break down the math behind this. In calculus, when we change variables in an integral, we introduce a Jacobian term. This principle applies here: $p(x) = p(z) * |dz/dx|$. Since $Z$ is uniformly distributed between 0 and 1, $p(z) = 1$. Thus, we have:''')
    st.markdown(r'''''')
    st.markdown(r'''$p(x) = |dz/dx|$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''A more interesting example is the Cauchy distribution, given by $p(x) = 1 / (\pi\sigma(1 + ((x-\mu)/\sigma)^2))$. The Cauchy distribution is particularly useful in many scenarios. It resembles a normal distribution near its peak but has much heavier tails, decaying as $1/x^2$ rather than exponentially. This makes it an excellent choice for a prior distribution when we want to allow for the possibility of extreme values.''')
    st.markdown(r'''''')
    st.markdown(r'''The CDF of the Cauchy distribution is $P(x) = 1/2 + (1/\pi)\arctan((x-\mu)/\sigma)$. Inverting this gives us $x = \mu + \sigma\tan(\pi(z-1/2))$. By generating uniform random numbers and applying this transformation, we can sample from a Cauchy distribution.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''That's why in our next session, we'll explore more advanced sampling techniques that can handle even more complex distributions. We'll look at rejection sampling and importance sampling, which can work in cases where the inverse CDF method isn't applicable. These methods will set the stage for our discussion of Markov Chain Monte Carlo techniques in the following lecture.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\mathbb{E}[f] \approx (1/L) * \sum f(z^{(l)})$, where $z^{(l)} \sim p(z)$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\mathbb{E}[f] = \int f(z) p(z) dz$''')
    st.markdown(r'''      $= \int f(z) (p(z) / q(z)) q(z) dz$''')
    st.markdown(r'''      $\approx (1/L) * \sum (p(z^{(l)}) / q(z^{(l)})) f(z^{(l)})$, where $z^{(l)} \sim q(z)$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\mathbb{E}[f] = (Z_q / Z_p) * \int f(z) (\tilde{p}(z) / \tilde{q}(z)) q(z) dz$''')
    st.markdown(r'''''')
    st.markdown(r'''The ratio $Z_q / Z_p$ can be estimated as:''')
    st.markdown(r'''''')
    st.markdown(r'''$Z_p / Z_q \approx (1/L) * \sum (\tilde{p}(z^{(l)}) / \tilde{q}(z^{(l)}))$, where $z^{(l)} \sim q(z)$''')
    st.markdown(r'''''')
    st.markdown(r'''Putting this all together, we get our final importance sampling estimator:''')
    st.markdown(r'''''')
    st.markdown(r'''$\mathbb{E}[f] \approx (\sum \tilde{r}_l f(z^{(l)})) / (\sum \tilde{r}_l)$, where $\tilde{r}_l = \tilde{p}(z^{(l)}) / \tilde{q}(z^{(l)})$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$p(z) = (N(z|0,1) * 1(z \in [5,\infty))) / \int_5^\infty N(z|0,1) dz$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\tilde{r} = \tilde{p}(z) / q(z) = 1(z \in [5,\infty))$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$E[f] \approx (\Sigma z_l * 1(z_l \in [5,\infty))) / (\Sigma 1(z_l \in [5,\infty)))$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\tilde{r} = \tilde{p}(z) / q(z) = N(z_l|0,1) / \exp(5 - z_l)$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''As we wrap up today's lecture, let's reflect on what we've learned and why it's crucial for your understanding of machine learning and scientific research.''')
    st.markdown(r'''''')
    st.markdown(r'''First and foremost, we've explored why sampling is such an integral part of machine learning. Many of you may have encountered these sampling techniques before, but the key insight from today's lecture is understanding the deep connection between sampling and the core objectives of machine learning. Fundamentally, machine learning aims to do two things: estimate posteriors and generate predictive distributions.''')
    st.markdown(r'''''')
    st.markdown(r'''Now, you might think that in science, we're only interested in fitting models and estimating parameters, not in making predictions. But this is a common misconception. Science is inherently predictive. Even if your primary goal is to estimate cosmological parameters like $\Omega_m$ or $w$, you still need to predict what observations you should see given these parameters. This is how we test theories like $\Lambda$CDM. And these predictions invariably involve complex integrals that require sophisticated sampling techniques.''')
    st.markdown(r'''''')
    st.markdown(r'''It's important to understand that the simple cases we covered in the first five lectures of this course are not representative of real-world research problems. You might think these simple cases are too hard for practical use, but it's actually the opposite. They're simple enough that we can solve the integrals analytically. However, these analytically solvable models often lack the expressiveness needed for real-world phenomena. That's why in research, we turn to methods like Markov Chain Monte Carlo (MCMC) for sampling. The key is to connect the principles we've learned from these simpler cases to the more complex methods used in actual research.''')
    st.markdown(r'''''')
    st.markdown(r'''In today's lecture, we covered several fundamental sampling techniques. We learned about the inverse Cumulative Distribution Function (CDF) method, which allows us to sample exactly from a distribution if we can integrate it. For low-dimensional cases where exact integration isn't possible, we explored rejection sampling, which lets us "carve out" the desired distribution from a simpler one. Building on this idea, we then discussed importance sampling, which provides a way to estimate integrals using samples from a different, more convenient distribution.''')
    st.markdown(r'''''')
    st.markdown(r'''We also introduced the crucial concept of effective sample size. While sampling is powerful, it's not just about running your computer for a long time. It's about generating samples efficiently, much like trying to estimate $\pi$ by throwing as few needles as possible while still getting an accurate estimate. This idea of effective sample size will become even more critical when we delve into MCMC methods in future lectures.''')
    st.markdown(r'''''')
    st.markdown(r'''The techniques we've covered today – inverse CDF, rejection sampling, and importance sampling – might seem basic, but they form the foundation for more advanced methods. Understanding these fundamentals is crucial for grasping the more complex sampling techniques used in cutting-edge machine learning and scientific research.''')
    st.markdown(r'''''')
    st.markdown(r'''Now that we've covered the theoretical aspects of sampling, let's turn our attention to the upcoming tutorial. This practical exercise will give you a chance to apply the concepts we've discussed and see firsthand how sampling techniques can be used to solve real-world problems in astrophysics.''')
    st.markdown(r'''''')
