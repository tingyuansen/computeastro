...
def show_page():
    ...
    st.markdown(r'''Welcome back, everyone. As we progress through this course, we're seeing a natural decrease in attendance, which is to be expected. Let's begin by recapping what we've covered in the last four lectures.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''As the slide mentions, recall from our lecture on logistic regression that even without an analytic closed form, we can optimize and find the best $w^*$ that minimizes the loss through stochastic gradient descent. The key is that as long as we can calculate the gradient $\nabla E(w)$, we can perform the optimization.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''   This is probably the first method that comes to mind. If we want to calculate $df/dx$, we can approximate it using $(\Delta f / \Delta x)$ with some finite $\Delta$. The slide mentions this as a numerical approximation based on the limit definition. However, this method has significant drawbacks:''')
    ...
    st.markdown(r'''   - As the slide notes, it's inefficient, equations can be verbose, and many derivatives are not easy to express symbolically.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''   The slide summarizes this beautifully:''')
    ...
    st.markdown(r'''   - It's perfect for our needs in neural network optimization. As the slide mentions, it's accurate and efficient.''')
    st.markdown(r'''''')
    st.markdown(r'''The only downside, as noted in the slide, is that it's numerical and doesn't give us a closed analytic form. But for neural network optimization using methods like stochastic gradient descent (SGD), this isn't critical at all.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''The slides show this beautifully as a computation graph:''')
    st.markdown(r'''''')
    st.markdown(r'''[Computation graph from the slides]''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\partial l/\partial W = \partial l/\partial h * \partial h/\partial z * \partial z/\partial y * \partial y/\partial W$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''1. Forward Pass: We calculate all the intermediate values. Given $W$, $x$, and $b$, we compute $y$, $z$, and $h$. The slides call this "dynamic programming" - we evaluate all the intermediate variables and store them.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''In practice, we often use libraries like PyTorch rather than implementing backpropagation ourselves. However, implementing a neural network using just NumPy can be very illuminating and help you understand these concepts at a deeper level, which we will do that in our tutorial.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Let's look at the dimensions more closely, as shown in the slides:''')
    st.markdown(r'''''')
    st.markdown(r'''$\partial L/\partial W = \partial L/\partial h * \partial h/\partial z * \partial z/\partial y * \partial y/\partial W$''')
    st.markdown(r'''$(1,K) * (K,K) * (K,M) * (M,MD) = (1,MD)$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''''')
    st.markdown(r'''Before we transition to the practical tutorial, I'd like to address a question that was raised:''')
    st.markdown(r'''''')
