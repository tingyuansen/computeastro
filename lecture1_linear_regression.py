...
def show_page():
    ...
    st.markdown(r'''''')
    st.markdown(r'''Our course webpage is computeastro.com. We'll have both in-person and Zoom lectures, with recordings available on the website. Office hours are Thursdays from 2:30-3:00 PM. This course is programming-intensive with research-based assignments. And yes, you can use ChatGPT and Copilot for my assignments - though they might not be the magic bullet you're expecting!''')
    st.markdown(r'''''')
    st.markdown(r'''We'll kick things off by delving into some fundamental machine learning concepts, including the distinction between supervised and unsupervised learning. We'll also cover cross-validation - a crucial skill for evaluating and selecting the best models.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''As I mentioned on Tuesday, you probably won't need a textbook for the second half of this course, as most of our lectures should be self-contained. However, we do assume some understanding of probability and linear algebra concepts.''')
    st.markdown(r'''''')
    st.markdown(r'''For those who need a refresher or want to brush up on their math skills, I recommend the book on the left of our slide: "Mathematics for Machine Learning." It's an excellent resource for the basics. For those seeking a more in-depth understanding, the textbook on the right, which we use for COMP 4670 from Bishop, is highly recommended. It's a timeless textbook that covers all the key ideas in classical machine learning, which forms a significant part of our course content.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Dimensionality Reduction: This is all about compressing information from high-dimensional data into fewer dimensions while keeping the essential features. In this course, we'll cover Principal Component Analysis (PCA), a go-to technique for this task.''')
    st.markdown(r'''''')
    st.markdown(r'''Clustering: Here, we're looking to find natural groupings within the data.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Looking ahead, here's our game plan for the upcoming lectures: Today, we're tackling regression (supervised learning). Next Tuesday, we'll dive into classification (also supervised learning). After that, we'll explore unsupervised learning, covering both clustering and dimensionality reduction''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Underfitting: This occurs when we use too few parameters or a very rigid model. The model is too simple to capture the underlying pattern in the data.''')
    st.markdown(r'''''')
    st.markdown(r'''Overfitting: On the other extreme, if we use too many parameters, our model might start fitting the noise in the data rather than the true underlying pattern.''')
    st.markdown(r'''''')
    st.markdown(r'''Good fit: This is the sweet spot where our model captures the true pattern without being overly influenced by noise.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Training set: This is used to fit the polynomial (or any model).''')
    st.markdown(r'''''')
    st.markdown(r'''Validation set: This is used to check the trend of what we call the loss function and to select the best model.''')
    st.markdown(r'''''')
    st.markdown(r'''Test set: This final set is used to evaluate the chosen model's performance on completely unseen data.''')
    st.markdown(r'''''')
