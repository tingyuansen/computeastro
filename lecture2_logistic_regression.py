...
def show_page():
    ...
    st.markdown(r'''''')
    st.markdown(r'''Welcome back, everyone. Last week, we discussed linear regression, where we built a linear model for a supervised task to make inferences on continuous variable labels. Today, we're extending our discussion of linear models to a different supervised task: classification.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''We'll start with the simple case of binary classification (e.g., cats vs. dogs) and then extend our discussion to multi-class problems. In today's tutorial, we'll focus on binary classification, but in your assignment (question 2), you'll have the opportunity to work on multi-class classification.''')
    st.markdown(r'''''')
    st.markdown(r'''Now, let's delve into the concept of classification. As we discussed earlier, the main difference between today's topic and last Thursday's is that we're dealing with a classification task. Like regression, classification is a supervised task, but instead of predicting continuous values, we're assigning input data to discrete classes.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''In fact, you'll explore this concept in your third assignment, where you'll work with generative models of quasar spectra. This will allow you not only to classify spectra but also to detect outliers - potentially leading to new discoveries.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''The goal in logistic regression is to find a discriminant function - a linear boundary in the feature space that separates class 1 from class 0. You might be tempted to approach this like we did with linear regression last week, using a simple linear model $y = w^T x$ to predict labels. However, this approach has significant drawbacks in classification tasks.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''If manually engineering features seems daunting, don't worry. In the coming weeks, we'll discuss methods that can automatically learn useful feature representations:''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''While this analytic gradient might seem trivial now, its significance becomes apparent in the context of neural networks and automatic differentiation, topics we'll explore in future lectures. In those cases, all functions are differentiable, but gradients are computed algorithmically. For logistic regression, however, we have this convenient closed-form solution.''')
    st.markdown(r'''''')
    st.markdown(r'''And voila. ''')
    st.markdown(r'''''')
    st.markdown(r'''The "master algorithm" of machine learning can handle about 90% of jobs. Here's a typical workflow:''')
    st.markdown(r'''''')
    st.markdown(r'''1. Your supervisor gives you a task.''')
    st.markdown(r'''''')
    st.markdown(r'''2. You think, "Okay, I'll first come up with some features and labels."''')
    st.markdown(r'''''')
    st.markdown(r'''3. Try logistic regression.''')
    st.markdown(r'''''')
    st.markdown(r'''4. 90% of the time, this will work. Done!''')
    st.markdown(r'''''')
    st.markdown(r'''5. If it doesn't work, repeat: find new features, try logistic regression again.''')
    st.markdown(r'''''')
    st.markdown(r'''6. If after a few attempts it still doesn't work, consider more complex models like neural networks or Gaussian processes.''')
    st.markdown(r'''''')
    st.markdown(r'''Now, let's discuss extending logistic regression to multiple classes, which is relevant for assignment three, question two.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''In some cases, we can't tolerate false positives, while in others, we can't tolerate false negatives. This boundary depends on the specific problem, and you'll explore this further in assignment three.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''This weighting is more of an art than a science, depending on the decision you want to make. You'll explore this in your assignment as well.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''This is still an active area of research. For example, here is an paper introduced concepts like "conservative prediction" or "confidence minimization," suggesting that we should only make predictions when we're very confident, which might be a good read if you are interested in topic as such.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''In our next lecture, we'll delve into unsupervised learning techniques, exploring how we can find patterns and structure in data without explicit labels. This will include topics like clustering and dimensionality reduction, which are crucial in many astronomical applications.''')
    st.markdown(r'''''')
    st.markdown(r'''Thank you for your attention, and I look forward to seeing you in the next class. Don't forget to work on your assignment, and feel free to reach out during office hours if you have any questions!''')
    st.markdown(r'''''')
    st.markdown(r'''''')
    st.markdown(r'''''')
