...
def show_page():
    ...
    st.markdown(r'''Welcome back, everyone. As we approach the end of the semester, I know you're all busy, but today marks our last lecture covering new material. Initially, I had planned to discuss Large Language Models (LLMs), but I've decided against it since it won't be on the exam. Given that many of you are finding the course challenging, I thought it would be more beneficial to use our next session for revision and to address any questions you might have.''')
    st.markdown(r'''''')
    st.markdown(r'''Let's start by discussing the assignment scores. The grader should be sending these out soon. I'm pleased to see that most of you are performing well. Despite some concerns about the course being math-heavy, the median score is around 85, with a mean of 80. In fact, quite a few students scored between 85 and 100. This should give you an idea of the difficulty level to expect for the final assignment.''')
    st.markdown(r'''''')
    st.markdown(r'''Speaking of the final assignment, its format will be similar to Assignment 3. We won't be asking you to derive mathematical theorems, but rather to apply the concepts you've learned to real-life scenarios. These scenarios will be open-ended, requiring you to make connections between the course material and practical applications. You've likely realized by now that simply inputting questions into ChatGPT won't suffice. However, if you understand which techniques to use and how to handle the data, you should find the tasks straightforward.''')
    st.markdown(r'''''')
    st.markdown(r'''The final assignment will consist of three questions, totaling 40 points. My part will account for 30 points, while Christophe's section will cover the remaining 10. This maintains the balance we've had throughout the course, with 50 points from the first half and 50 from my half. The 30 points from my section will be spread across three main questions, each similar in length to those in Assignment 3. These will be broken down into smaller sub-questions, typically worth one or two points each. If you find yourself writing more than 50 lines of code for a two-point question, you're probably overthinking it. The focus is on understanding how to apply the concepts we've covered.''')
    st.markdown(r'''''')
    st.markdown(r'''Importantly, the final assignment will draw upon almost all the techniques we've studied in the course. I want you to think outside the box and combine different methods, much like you did in Assignment 3 when you had to use PCA with GMM. Expect to encounter problems that require you to integrate various techniques we've discussed.''')
    st.markdown(r'''''')
    st.markdown(r'''In terms of time management, since the final assignment is worth 30 points compared to Assignment 3's 20 points, you should plan to spend about 1.5 times the effort. If Assignment 3 took you two full days, allocate three full days for the final. If you could solve Assignment 3 in about an hour and a half (which is realistic if you understand the material well), then you should be able to complete the final in around two hours.''')
    st.markdown(r'''''')
    st.markdown(r'''Now, let's move on to today's lecture topic: Markov chains. This concept is widely applied in astronomy, particularly in the form of Markov Chain Monte Carlo (MCMC) methods. Often, we use these techniques without fully understanding the underlying principles, treating them as black boxes. However, many popular astronomy packages implementing these methods are simpler than you might think - often less than 50 lines of code.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Crucially, this method only requires us to evaluate the ratio of probabilities, not the absolute probabilities. In Bayesian terms, we're working with $p(z)/p(z') = [likelihood(z) * prior(z)] / [likelihood(z') * prior(z')]$. The normalization constants cancel out, which is why MCMC is so powerful for Bayesian inference where we often can't calculate the evidence term.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''In practice, the Metropolis algorithm proceeds as follows: You generate a new sample $z$ from $q(z|z')$, where $z'$ is your current position. Then you calculate $u$, a random number from a uniform distribution between 0 and 1. You accept the new sample if $p(z)/p(z') > u$. If accepted, you move to the new position $z$; if rejected, you stay at your current position $z'$. This process, repeated many times, will eventually sample from the target distribution $p(z)$, regardless of the starting point or the specific form of the symmetric proposal distribution.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''So this is the definition of the convergence, meaning that for any starting distribution, it does not matter. But when you run long enough, then all this population of people that you have signed up in, I don't know, like some open days of mom's travel, if you give them the travel guide, it will slowly become a Canberran. ''')
    st.markdown(r'''''')
    st.markdown(r'''But also, you want to make sure that the outcome is unique. When this converges into some distribution, it converges into one distribution. And the uniqueness is good enough here because we already showed that if you fulfill the detailed balance, becoming a Canberran is one of the outcomes. If the outcome is unique, then you are done. Then you know that you would converge to the distribution. ''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''This term might sound familiar to those who've studied statistical mechanics, but don't worry if you haven't - I first learned it in my second year and have been using it to sound nerdy ever since. I often joke that "I explore Canberra ergodically."''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$A(z', z) = min(1, (p(z) q(z'|z)) / (p(z') q(z|z')))$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''   $= q(z|z') min(1, (p(z) q(z'|z)) / (p(z') q(z|z'))) p(z')$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''This approach is particularly useful when you encounter high-dimensional problems. I've seen many cases where students come to me saying, "I ran MCMC for two days, and it's not converging!" Often, they're trying to sample 101 parameters when 100 of them are easy to sample analytically, and only one requires MCMC. By using Gibbs sampling, you can dramatically reduce computational time and resource usage.''')
    st.markdown(r'''''')
    st.markdown(r'''I know many people are super lazy. They see a posterior, write down the likelihood, and just run MCMC, burning out all their computing time without a care. Don't do that! Sometimes, just writing two more lines of code can save your time, computing resources, and the Earth.''')
    st.markdown(r'''''')
