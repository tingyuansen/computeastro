...
def show_page():
    ...
    st.markdown(r'''''')
    st.markdown(r'''## From Basic Machine Learning to Neural Networks''')
    st.markdown(r'''''')
    st.markdown(r'''In our previous sessions, we discussed neural networks. To put this in context, let's remind ourselves why we use neural networks in the first place. We began this course with several lectures on the basics of machine learning. You can think of these fundamental techniques as learning to play basic instruments - guitar, piano, or violin. While these methods are useful, they each have their limitations.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''## Introducing Gaussian Processes''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''## Overcoming Limitations of Linear Regression''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''## Course Structure and Resources''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''## Applications of Gaussian Processes in Astronomy''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### Exoplanet Transits''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### Quasar Accretion''')
    st.markdown(r'''''')
    st.markdown(r'''A more compelling example is the accretion of black holes, also known as quasar accretion. You might recall the spectra example from earlier in the course, but another way to study black holes is by examining their light curves. This is, in fact, one of the primary tasks of sky surveys.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### Asteroseismology''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''## Advantages of Gaussian Processes in Astronomy''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''## Overview of Machine Learning Concepts''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''## Gaussian Processes vs. Neural Networks''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''## Mathematical Foundations of Gaussian Processes''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''## Introduction to Kernel Methods''')
    st.markdown(r'''''')
    st.markdown(r'''So, what if we don't know how to define the features? What assumptions can we make? A naive assumption might be that we should only consider neighboring points. For example, if we want to predict the weather at a certain location, instead of writing a complex function relating various variables to temperature, we might simply estimate it based on readings from neighboring areas. This approach, similar to K-nearest neighbors, assumes some continuity in the target function based on similarity.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### Kernel Density Estimation''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''One straightforward approach is to place a Gaussian kernel around each data point and sum their contributions. The formula for KDE is:''')
    st.markdown(r'''''')
    st.markdown(r'''$p(x) = \dfrac{1}{N} \sum_{n=1}^N \dfrac{1}{(2\pi h^2)^{1/2}} \exp\bigg(-\dfrac{\|x - x_n\|^2}{2h^2}\bigg)$''')
    st.markdown(r'''''')
    st.markdown(r'''Here, $N$ is the number of data points, and $h$ is a smoothing parameter. For astronomers, this concept of a smoothing scale should be familiar from N-body simulations and other contexts.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''## Kernel Methods for Regression''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$y(x) = \sum^N_{i=1} k(x, x_i)a_i$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### Defining Similarity: The Kernel Function''')
    st.markdown(r'''''')
    st.markdown(r'''Given this framework, our first task is to define the form of similarity. How do we determine if two input points are close, especially in high-dimensional spaces? One obvious choice, which you might recall from recent discussions, is the dot product.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$K = \Phi(x)\Phi(x)^T \in \mathbb{R}^{N×N}$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### Reformulating Linear Regression with Kernels''')
    st.markdown(r'''''')
    st.markdown(r'''Now, you might wonder: Why are we doing this? The goal is to rewrite our linear regression solution in terms of this kernel matrix $K$, rather than the feature matrix $\Phi$. This approach allows us to work with similarities between data points directly, without needing to explicitly define or compute the feature vectors.''')
    st.markdown(r'''''')
    st.markdown(r'''For simplicity, let's assume we're working without regularization and noise ($\lambda = 0$, $\beta^{-1} = 0$). The general proof with these terms included is similar and can be found in standard textbooks.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Let's apply the kernel trick to linear regression. We know that in linear regression, the best weights are given by $w* = (\Phi^T \Phi)^{-1} \Phi^T t$. For any new data point $x$, the prediction is $y(x) = \phi(x)^T w*$. Substituting $w*$, we get:''')
    st.markdown(r'''''')
    st.markdown(r'''$y(x) = \phi(x)^T (\Phi^T \Phi)^{-1} \Phi^T t$''')
    st.markdown(r'''''')
    st.markdown(r'''However, this isn't in the form we want because our kernel is $\phi\phi^T$, not $\phi^T \phi$. Can we reformulate this in terms of $\phi\phi^T$? The answer is yes.''')
    st.markdown(r'''''')
    st.markdown(r'''First, we use the matrix inversion lemma: $(AB)^{-1} = B^{-1} A^{-1}$. We can see that these two terms will cancel out. But we still don't see $\phi\phi^T$. So we'll inject $\phi\phi^T$ here, injecting an identity. We'll inject both $\phi\phi^T$ as well as $(\phi\phi^T)^{-1}$. This is just injecting an identity; we're doing nothing. But this term will cancel out. Grouping all these terms, we arrive at:''')
    st.markdown(r'''''')
    st.markdown(r'''$y(x) = (\Phi\phi(x))^T (\Phi\Phi^T)^{-1} t$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''1. Symmetry: $k(x,y) = k(y,x)$ for all $x$ and $y$. This is intuitive; the similarity between two points should be the same regardless of the order in which we consider them.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''I know this is a bit much, but it's important to understand. It just means that you can't find any bivariate function and call it a similarity function. You need to satisfy these two things. If you satisfy these two things, then by Mercer's condition, there exists a $\phi$ such that the dot product of $\phi$ is exactly the kernel.''')
    st.markdown(r'''''')
    st.markdown(r'''### Examples of Kernel Functions''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### The Radial Basis Function (RBF) Kernel''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$k(x_m, x_n) = \exp\bigg(-\dfrac{\|x_n - x_m\|^2}{2\sigma^2}\bigg)$''')
    st.markdown(r'''''')
    st.markdown(r'''This kernel has an intuitive interpretation: it measures the similarity between points $x_n$ and $x_m$ based on their distance, modulated by a length scale $\sigma$. If we're comparing the similarity between two points in space, for instance, we'd take the distance between the two and adjust it by some length scale $\sigma$, which defines the "influence sphere".''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\exp(x) = 1 + x + \dfrac{x^2}{2!} + \dfrac{x^3}{3!} + ...$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''This is why Gaussian processes are so appealing to statisticians and astronomers alike. We're not explicitly defining a model, yet it's as if we've defined a model with infinite-dimensional freedom to fit our data. That's the power of kernel methods and Gaussian processes.''')
    st.markdown(r'''''')
    st.markdown(r'''It's natural if this seems overwhelming at first. It took many researchers a long time to fully internalize these concepts. But as we progress, you'll see how these ideas come together to form a powerful framework for machine learning in astronomy.''')
    st.markdown(r'''''')
    st.markdown(r'''### Kernelizing Bayesian Linear Regression''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''The magic here is similar to what we've seen before. We know that $m'$ (the mean of the predictive distribution) is given by:''')
    st.markdown(r'''''')
    st.markdown(r'''$m' = \phi^T(x^*)(\Phi^T \Phi)^{-1}\Phi^T t$''')
    st.markdown(r'''''')
    st.markdown(r'''Using the identity tricks we've learned, this simplifies to the kernel form we've just described.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### The Key Result of Gaussian Process Regression''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''It's not too surprising when you think about it. If you want to predict a physical quantity at a new location in space, you might find the nearest neighbors and calculate some form of average. This is an extension of that idea, but with a rigorous probabilistic foundation.''')
    st.markdown(r'''''')
    st.markdown(r'''### The Function-Space Perspective of Gaussian Processes''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### Understanding Kernels in Gaussian Processes''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$k(x_n, x_m) = \theta_0 \exp(-\dfrac{\theta_1}{2} ||x_n - x_m||^2) + \theta_2 + \theta_3 x_n^T x_m$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''These parameters have important consequences. If we fix different length scales and always observe the same training data, we get different predictions. If we choose the optimal length scale, we get a function that makes sense. But if we choose a length scale that's too large or too small, we'll get predictions that don't make much sense - we'll either overfit or underfit.''')
    st.markdown(r'''''')
    st.markdown(r'''### Comparing Gaussian Processes and Bayesian Linear Regression''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### Practical Application of Gaussian Processes''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''$S' = k(x^*, x^*) - k(x^*, X) k(X, X)^{-1} k(X, x^*)$''')
    st.markdown(r'''''')
    st.markdown(r'''We've seen this many times. It's straightforward: you have some training data $X$ and $t$. For now, we're assuming no noise, but we'll discuss how to include noise in the model later.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Where $L$ is a lower triangular matrix. Why is this good? Because Cholesky decomposition requires $\dfrac{N^3}{3}$ operations. It's still $O(N^3)$, but it's much better than other methods. For example, other ways to do the inverse usually take about two times $\dfrac{N^3}{3}$ operations. So Cholesky is still the best you can do if you want to blindly invert the matrix. If you have a choice between one hour or three hours of computation, you'll choose one hour, right? It's not a massive gain, but it's significant.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''1. Compute the Cholesky decomposition: $L \equiv \mathrm{Cholesky}(k(X, X))$''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''The key takeaway is that it's really just about how to best perform the matrix inversion. Even if you don't optimize it perfectly, you can still get good results. Define the kernel matrix, do the linear algebra operations, and you'll be fine.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### Optimizing Hyperparameters in Gaussian Processes''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\ln p(t | X, \theta) = -\dfrac{1}{2} \ln |K| - \dfrac{1}{2} t^T K^{-1} t - \dfrac{N}{2} \ln(2\pi)$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\dfrac{\partial}{\partial \theta_i} \ln p(t | X, \theta) = -\dfrac{1}{2} \mathrm{Tr}(K^{-1} \dfrac{\partial K}{\partial \theta_i}) + \dfrac{1}{2} t^T K^{-1} \dfrac{\partial K}{\partial \theta_i} K^{-1} t$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\ln p(t | X, θ) = -\dfrac{1}{2} \ln |K| - \dfrac{1}{2} t^T K^{-1} t - \dfrac{N}{2} \ln(2\pi)$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### Tutorial: Implementing Gaussian Processes''')
    st.markdown(r'''''')
    st.markdown(r'''Now, let's walk through a practical example. We'll spend some time on this specifically because it's crucial for understanding the practical applications of Gaussian Processes.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''### Tutorial: Quasar Light Curves''')
    st.markdown(r'''''')
