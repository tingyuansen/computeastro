...
def show_page():
    ...
    st.markdown(r'''Welcome back. Let me recap what we covered last time. We talked about why we need Gaussian processes, as they extend linear regression. Linear regression requires defining features, which can be challenging, making it quite rigid. This is why we consider using neural networks. However, neural networks have the shortcoming of being difficult to make Bayesian, meaning we can find the best neural network, but it's hard to quantify the uncertainty of the networks.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Despite their limitations, Gaussian processes are incredibly valuable in astronomy, particularly because we often work with limited training data and low-dimensional observations. Common applications include analyzing time series or examining photometric data, both of which typically involve a small number of features and training data points. In today's tutorial, we'll explore star-galaxy separation, which is an excellent example of how Gaussian process classification can be used to distinguish between different types of stars. You'll see that all of this can be accomplished using the methods we'll cover today.''')
    st.markdown(r'''''')
    st.markdown(r'''Gaussian process classification is arguably the best method for many astronomical classification tasks, such as finding metal-poor stars from sky mapper data. It's flexible and provides uncertainty estimates. Once you understand Gaussian process classification, it's just a few lines of code to implement one of the best classification methods available.''')
    st.markdown(r'''''')
    st.markdown(r'''However, I should mention that this is probably one of the driest lectures because teaching Gaussian process classification involves a lot of derivations without much room for inspiration. It's mostly hardcore derivations, but I can explain why we need to do them.''')
    st.markdown(r'''''')
    st.markdown(r'''Before we dive into Gaussian processes, let's revisit the logistic regression we covered in the second lecture. Logistic regression is the linear version for classification, and just as Gaussian process regression extends linear regression, Gaussian process classification extends logistic regression. Last time, we didn't discuss the Bayesian version of logistic regression; we only focused on finding the best boundary between classes. Today, we'll characterize the uncertainty of the boundary and explain why that's important.''')
    st.markdown(r'''''')
    st.markdown(r'''We skipped this aspect initially because it requires learning about Laplace approximation, which is quite dry. But once you understand these concepts, along with the linear algebra and linear regression we've covered, applying the kernel trick to create a Gaussian process becomes straightforward.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\ln f(z) \approx \ln f(z_0) - (1/2)A(z - z_0)^2$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$f(z) \approx f(z_0) \exp\{-(A/2)(z - z_0)^2\}$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$q(z) = (A/(2\pi))^{(1/2)} \exp\{-(A/2)(z - z_0)^2\}$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$q(z) = |A|^{1/2} / (2\pi)^{M/2} \exp\{-(1/2)(z - z_0)^T A(z - z_0)\} = \mathcal{N}(z | z_0, A^{-1})$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Any questions so far? I know this is quite dry, but hopefully, when we get to the end of the derivation, you'll start to see what it actually means in practice.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$-\ln p(w | t, X) = (1/2)(w - m_0)^T S_0^{-1}(w - m_0) - \sum^N_{n=1} \{t_n \ln y_n + (1 - t_n)\ln(1 - y_n)\} + const.$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$\int \Phi(\lambda a) \mathcal{N}(a | \mu, \sigma^2) da = \Phi(\mu / \sqrt{\lambda^{-2} + \sigma^2})$''')
    st.markdown(r'''''')
    st.markdown(r'''This result is crucial for our purposes. While it's good to see the proof once, for this course, it's more important that you understand how to implement it rather than the details of the derivation. Nevertheless, let's walk through the proof briefly.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''   $P(Z \leq 0) = \Phi(\mu / \sqrt{\omega^2 + \sigma^2})$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Our ultimate goal is to infer $t^*$ given $x^*$, $t$, and $X$. However, we can't calculate this directly. The derivation involves several important mathematical tricks, which, while somewhat dry, are crucial for understanding this type of problem-solving approach. These techniques will be particularly important when we discuss MCMC and sampling from posteriors in future lectures.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''Where $m(x^*) = k(x^*, X)K^{-1}a$ and $\sigma^2(x^*) = k(x^*, x^*) - k(x^*, X)K^{-1}k(X, x^*)$.''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''$p(t^* = 1 | x^*, X, t) \approx \sigma((1 + \pi d^2/8)^{-1/2} c)$''')
    st.markdown(r'''''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''If you forget all the math we've covered in the last 70 minutes, there's really just one slide you need to remember. This slide contains the practical implementation of Gaussian Process Classification (GPC), which is what you'll use in your assignments. But I think what you've learned is also very practical because it's all about how you calculate some posterior that you have no direct access to, including the use of dummy variables. These are techniques you might encounter in research, so it's actually not as scary as it might seem at first glance.''')
    st.markdown(r'''''')
    st.markdown(r'''I really dislike dry lectures because they often lack insights. In this case, however, there's only one way to solve this problem. If you see this formulation and you've done enough statistics, you'll recognize that this is the only approach you can take. The logic is straightforward: you have a term you don't know how to handle, so you keep introducing dummy variables until you know all the terms. But as you increase the dummy variables, you encounter integrals you can't solve analytically. So, most likely, you approximate distributions as Gaussian. Hopefully, you can then do some integration. And if you encounter a weird function like a sigmoid, you transform it into a probit function.''')
    st.markdown(r'''''')
    st.markdown(r'''Let's recap the key result of GPC. The predictive distribution is given by:''')
    st.markdown(r'''''')
    st.markdown(r'''$p(t^* = 1 | x^*, X, t) = \sigma((1 + \pi d^2/8)^{-1/2} c)$''')
    st.markdown(r'''''')
    st.markdown(r'''where:''')
    st.markdown(r'''$c = k(x^*, X)(t - \sigma(a_{MAP}))$''')
    ...
    st.markdown(r'''''')
    st.markdown(r'''This approach to GPC is a rare case where we can use these approximation tricks and still get excellent results. In many other scenarios, these integrals become impossible to solve analytically, which is why we'll be learning about sampling and MCMC in the next lectures. MCMC isn't just something people know about for fun. It's developed because as you move from simple linear regression to more complex models like Gaussian processes, you encounter these nasty integrals that you have no way to solve analytically, even when you're just trying to compute a posterior or predictive distribution.''')
    st.markdown(r'''''')
    st.markdown(r'''In conclusion, GPC is a powerful tool that provides flexible decision boundaries and uncertainty estimates, making it ideal for many astronomical classification tasks. It marvels at finding boundaries without explicitly defining features, thanks to the kernel trick. And it gives us a Bayesian version of classification, allowing us to quantify the uncertainty of the boundary based on the density of the training data. Understanding its derivation gives you insights into handling complex statistical problems, even if in practice you'll mostly be using the final formula we derived today.''')
    st.markdown(r'''''')
    st.markdown(r'''''')
